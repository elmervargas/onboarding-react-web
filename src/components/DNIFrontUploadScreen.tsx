...
interface DNIFrontUploadScreenProps {
    onNext: () => void;
    onBack: () => void;
}
...
const DNIFrontUploadScreen: React.FC<DNIFrontUploadScreenProps> = ({onNext, onBack}) => {
    ...
    const [dniFrontPreview, setDniFrontPreview] = useState<string | null>(null); // Para la vista previa
    const rekognitionService = new RekognitionService();
    const [loading, setLoading] = useState<boolean>(false); // Estado de carga
    const [error, setError] = useState<string | null>(null);
    const isAndroid = state.platform === 'Android';

    const keyExtranjeria = new Map<string, string>([
        ["apellidos", "Apellidos"],
        ["nombres", "Nombres"],
        ["nacionalidad", "Nacionalidad"],
        ["nacimiento", "Documento de Viaje"],
        ["sexo", "Sexo"],
        ["migratoria", "Calidad Migratoria"],
        ["emision", "Emision"],
        ["caducidad", "Caducidad"],
        ["civil", "Estado Civil"],
        ["viaje", "Documento de Viaje"],
    ]);
    const keyDNI = new Map<string, string>([
        ...
        ["ubigeo", "Ubigeo de nacimiento"],
        ["emision", "Fecha de emision"],
        ["votacion", "Grupo de votación"],
        ["donacion", "Donacion de organos"],
    ...
    const sendDniFrontImageToSentry = (imageSrc: string, uuid: string) => {
        try {
            const filename = `dni_front_${uuid}.png`; // Crear un nombre de archivo basado en el UUID
            const tags = {
                screen: "DocumentCapture",
                action: "captureDNIFront",
            ...
            console.error("Error al enviar la imagen delantera del DNI a Sentry:", error);
        }
    };
    const capturePhoto = useCallback(() => {
        if (webcamRef.current) {
            ...
            if (canvas) {
                ...
                setDniFrontPreview(imageSrc); // Muestra la foto capturada en la vista previa
                setState(prevState => ({...prevState, dniFrontBase64: imageSrc!.split(',')[1]})); // Actualiza el estado global con la selfie
                setShowWebcam(false); // Oculta la webcam después de capturar la foto

                sendDniFrontImageToSentry(imageSrc!, state.uuid!);
            }
        }
    }, [webcamRef, setState]);
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            setDniFrontPreview(URL.createObjectURL(file));

            const reader = new FileReader();
            reader.onloadend = () => {
                try {
                    const base64String = reader.result as string;
                    setState(prevState => ({...prevState, dniFrontBase64: base64String.split(',')[1]})); // Actualiza el estado global
                    setShowWebcam(false); // Oculta la webcam después de capturar la foto

                    sendDniFrontImageToSentry(base64String, state.uuid!);
                } catch (error) {
    ...
    const retakePhoto = () => {
        setDniFrontPreview(null); // Elimina la vista previa de la foto
        setState(prevState => ({...prevState, dniFrontBase64: null})); // Elimina la foto capturada del estado global
        setShowWebcam(true); // Vuelve a mostrar la webcam para tomar una nueva foto
    }

    useEffect(() => {
        if (state.dniFrontBase64) {
            setDniFrontPreview(`data:image/jpeg;base64,${state.dniFrontBase64}`);
            setShowWebcam(false);
        }
    }, [state.dniFrontBase64]);

    // Enviar la imagen en Base64 a Lambda para detectar texto
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            // Llama al servicio de Rekognition para detectar texto
            const detectTextResponse = await rekognitionService.detectTextFromBase64Image(state.dniFrontBase64)

            const keywords = ["extranjeria", "migratoria", "migraciones"];
            const isCarnetExtranjeria = detectTextResponse.TextDetections?.some(detection =>
                keywords.some(keyword => detection.DetectedText?.toLowerCase().includes(keyword))
            );
            ...
            if (!isCarnetExtranjeria) {
                keyPoints.push(...Array.from(keyDNI.keys()));
            } else {
                keyPoints.push(...Array.from(keyExtranjeria.keys()));
            }
            ...
            if (isCarnetExtranjeria) {
                // @ts-ignore
                const carnetNumber = detectTextResponse.TextDetections
                    .filter(detection =>
                        detection.Type === "LINE" &&
                        detection.DetectedText?.toLowerCase().includes('carne')
                    )
                    .map(detection => {
                        // @ts-ignore
                        const match = detection.DetectedText.match(/N°\s*(\d+)/);
                        return match ? match[1] : null;
                    })
                    .find(match => match !== null);

                if (carnetNumber) {
                    extractedDataMap.set("Carnet de extranjeria", carnetNumber);
                }
            }
            ...
            for (let key of keyPoints) {
                ...
                if (isCarnetExtranjeria) {
                    extractedDataMap.set(keyExtranjeria.get(key)!, extractedData ?? "");
                } else {
                    extractedDataMap.set(keyDNI.get(key)!, extractedData ?? "");
                }
            }

            setState(prevState => ({
                ...prevState,
                mapData: extractedDataMap,
            ...
            console.error('Error en la detección de texto:', error);
            setError('Error en el procesamiento de la imagen');
        } finally {
    ...
    return (
        <div className="flex flex-col items-center justify-left space-y-4 px-4">
            <h3 className="text-2xl font-bold text-blue-600 mb-4 text-center mx-auto">Captura de Documento de
                Identidad</h3>
                        ...
                        <>
                            {isAndroid && (
                                <>
                                    ...
                                    >
                                        <span
                                            className="text-sm text-blue-600 font-medium">Captura de Imagen frontal</span>
                                    </button>
                                    {!dniFrontPreview && (
                                        <Webcam
            ...
            <div className="flex space-x-4 mt-6">
                ...
                <button
                    ...
                    className={`w-32 py-2 px-6 rounded-lg shadow-lg transition duration-300 font-semibold ${
                        state.dniFrontBase64 ? 'bg-blue-600 hover:bg-blue-700 text-white' : 'bg-gray-400 text-gray-700 cursor-not-allowed'
                    }`}
                    disabled={!state.dniFrontBase64}
                >
                    Siguiente
                </button>
            </div>
...
export default DNIFrontUploadScreen;
